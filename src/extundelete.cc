...
void load_super_block(ext2_filsys fs)
{
  ...
  // Sanity checks.
  assert(super_block.s_magic == 0xEF53);	// ext2-based fileysystem
...
void examine_fs(ext2_filsys fs)
{
  ...
  if (super_block.s_journal_inum)
  {
    ...
    blk_t blknum;
    errcode_t retval = ext2fs_bmap(fs, journal_ino, inode, NULL, 0, 0, &blknum);
    if(retval) std::cout << "bmap returned " << (int) retval << std::endl;
    ...
    io_manager io_mgr = unix_io_manager;
    errcode_t retval = ext2fs_open( commandline_journal_filename.c_str(),
                         EXT2_FLAG_JOURNAL_DEV_OK, 0, 0, io_mgr, &jfs);
    if(retval) {
      std::cout << "Error opening external journal." << std::endl;
      exit(1);
    }
    if((retval = io_channel_read_blk(jfs->io, jfs->super->s_first_data_block+1,
                                  -1024, buf) )) {
      com_err("extundelete", retval, "while reading journal superblock");
      exit(1);
    }
  }
  ...
  if (!commandline_restore_files.empty()) {
    std::ifstream infile;
    unsigned int namelen = 2560;  //Hopefully this is long enough
